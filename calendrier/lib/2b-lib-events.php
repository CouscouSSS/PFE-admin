...
class Events {
  ...
  /* [EVENT FUNCTIONS] */
  function get($id_user,$date) {
  // get() : get event for the selected date
  // PARAM $date : date
 
    $sql = "SELECT * FROM `events` WHERE id_user=? AND `date`=?";
    $evt = $this->fetch($sql, [$id_user,$date]);
    return count($evt)==0 ? false : $evt[0]['details'] ;
  }

  function getRange($id_user,$start, $end) {
  // getRange() : get all events in between selected date range
  // PARAM $start : start date
  //       $end : end date
    $sql = "SELECT * FROM `events` WHERE id_user=? AND `date` BETWEEN ? AND ?";
    $evt = $this->fetch($sql,[$id_user,$start, $end], "date", "details");
    return count($evt)==0 ? false : $evt ;
  }

  function save($date,$details,$id_user) {
  // save() : create/update event on specified date
  // PARAM $date : date
  //       $details : event details

    
      $sql = "REPLACE INTO `events` (`date`, `details`, `id_user`) VALUES (?, ?,?)";
      return $this->exec($sql,[$date,$details,$id_user]);
  }

  function delete($id_user,$date) {
  // delete() : delete event on specified date
  // PARAM $date : date
 
    $sql = "DELETE FROM `events` WHERE id_user=? AND `date`=?";
    return $this->exec($sql, [$id_user,$date]);
  }
